...
/**public abstract class Everything extends Rectangle{
	...
	public void OnCollision(Everything collidedObject)
	{
		...
		double angleOfCollision = Math.atan((collidedObject.y - this.y)/(collidedObject.x - this.x));
		//double totalEnergy =
		...
		collidedObject.setVelocity(new Vector2(collidedObject.Velocity.x + this.Velocity.x * (float) Math.cos(angleOfCollision) , collidedObject.Velocity.y +  this.Velocity.y * (float) Math.sin(angleOfCollision)));

		this.setVelocity(new Vector2(this.Velocity.x -  this.Velocity.x * (float) Math.cos(angleOfCollision) , this.Velocity.y - (float) this.Velocity.y * (float) Math.sin(angleOfCollision) ));
